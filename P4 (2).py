...
# ---
# 
# * Estudiante: **Axel Gabriel Hernández Masís**
# * Carné: **B73690**
# * Grupo: **1**
# 
# ---
...
#Sección 4.1
#Para esta sección se tomara el modulador, demodulador y se intentará pasar
#la imagen para que esta se replique y se expresen el paso de las imagen 
#por medio de los dos mencionados anteriormente.
"Modulación"
...
#Periodo de la señal
    ...
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)
    portadora_1 = np.sin(2*np.pi*fc*t_periodo) 
    portadora_2 = np.cos(2*np.pi*fc*t_periodo)
...
#Asignación de las formas de onda (QPSK)

    a=1

    for i, bit in enumerate(bits):
#Como hay 2 portadores, se necesitan 4 estados con lo cual ae realizan cuatro
#opciones:
    
    #La portadora 1 es positiva    
        if (bit == 1):
            if (a%2 != 0):
                senal_Tx[i*mpp : (i+1)*mpp] = portadora_1*1
                moduladora[i*mpp : (i+1)*mpp] = 1
                a = a+ 1

            if (a%2 == 0):
                senal_Tx[i*mpp : (i+1)*mpp] = portadora_2*1
                moduladora[i*mpp : (i+1)*mpp] = 1
                a = a+1         
            
#La portadora 1 es negativa si es cero
        if (bit == 0):
            if (a%2 != 0):
                senal_Tx[i*mpp : (i+1)*mpp] = portadora_1*-1
                moduladora[i*mpp : (i+1)*mpp] = 00
                a =a+1
...
# Demodulador
# Para el demodulador se utiliza la sección anterior
...
#Medición de gráficos
#Para imprimir las gráficas
import matplotlib.pyplot as plt

# Expresa el cambio entre señales
fig, (ax1, ax2, ax3, ax4) = plt.subplots(nrows=4, sharex=True, figsize=(14, 7))

# Muestra la onda cudrada
ax1.plot(moduladora[0:600], color='m', lw=2) 
ax1.set_ylabel('$b(t)$')

# EXpresa  señal modulada por BPSK
ax2.plot(senal_Tx[0:600], color='b', lw=2) 
ax2.set_ylabel('$s(t)$')

# Expresa la señal modulada
ax3.plot(senal_Rx[0:600], color='g', lw=2) 
ax3.set_ylabel('$s(t) + n(t)$')

# Expresa la señal demodulada
ax4.plot(senal_demodulada[0:600], color='r', lw=2) 
ax4.set_ylabel('$b^{\prime}(t)$')
ax4.set_xlabel('$t$ / milisegundos')
fig.tight_layout()
plt.show()
...
# Vector de tiempo 
vector_tiempo = np.linspace(0, 0.1,100)

Modulo=[1,-1]

# Formas de onda posibles 
X_t = np.empty((4, len(vector_tiempo)))	   # 4 funciones del tiempo x(t) 
...
for i in Modulo:
    x1 = i * np.cos(2*(np.pi)*fc*vector_tiempo) +  i* np.sin(2*(np.pi)*fc*vector_tiempo)
    x2 = -i * np.cos(2*(np.pi)*fc*vector_tiempo) +  i* np.sin(2*(np.pi)*fc*vector_tiempo) 
    X_t[i,:] = x1
    X_t[i+1,:] = x2
    plt.plot(vector_tiempo,x1, lw=2)
    plt.plot(vector_tiempo, x2, lw=2)       

# Promedio de las cuatro realizaciones por cada momento. 
P = [np.mean(X_t[:,i]) for i in range(len(vector_tiempo))]
plt.plot(vector_tiempo, P, lw=6,color='k',label='Promedio Realizaciones')

# Grafica  resultado teórico
E = np.mean(senal_Tx)*vector_tiempo  # Valor esperado de la señal 
plt.plot(vector_tiempo, E, '-.', lw=3,color='c',label='Valor teórico')

# Realizaciones, promedio practico y teórico
...
#Con la transformada de furier se busca la densididad de la señal
from scipy import fft

#Para generar la transformada:
senal_furier = fft(senal_Tx)

#Cantidad de Muestras de la señal
N_muestras = len(senal_Tx)

#Cantidad de  simbolos
N_simbolos = N_muestras//mpp

#Periodo de onda
T_periodo = 1 / fc

#Periódo de muestras
P_muestras = T_periodo / mpp

#Periodo de simulación
T_simulacion = N_simbolos * T_periodo

#Frecuencia
f = np.linspace(0.0, 1.0/(2.0*P_muestras),N_muestras//2)


#Gráfica
plt.plot(f, 2.0/N_muestras * np.power(np.abs(senal_furier[0:N_muestras//2]), 2))
plt.xlim(0, 20000)
plt.grid()
plt.show()
